# Gemini AI Assistant Instructions — bot.ts Monorepo Root

Welcome to the **bot.ts** monorepo! This file serves as a foundational guide for any AI assistant (including Gemini CLI) to understand the codebase structure, architectural rules, development workflows, and constraints.
...
## 3. Custom Commands (Gemini CLI)

As a Gemini CLI agent, you are authorized and expected to use the project's standard development commands during your research and execution phases:

* **Format**: `bun run format` (runs Biome formatting check & fix)
* **Typecheck**: `bunx turbo check` (runs `tsc --noEmit` across all workspace projects)
* **Test**: `bun run test` (runs unit and framework offline tests)
