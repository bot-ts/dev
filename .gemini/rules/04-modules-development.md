# Domain 4: Custom Modules Development
...
## 1. Development & Publishing Layout (Clean package)
...
## 2. Installation Layout (Flat local directory)
...
### The `unpackNpmModule` Helper:
When writing or refactoring CLI commands for installing or updating modules, **always** use the unified `unpackNpmModule` helper function from `#src/util` to automate this extraction and mapping cleanly:

```typescript
import { unpackNpmModule } from "#src/util"

unpackNpmModule(packageName, version, destPath)
```
This is fully cross-platform and handles downloading, untarring, and flat extraction safely under the hood.
